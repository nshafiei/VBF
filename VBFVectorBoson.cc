...
void VBFVectorBoson::bookHistograms(){
  ...
  ht->addHist("CenteraletaVphi",       new TH2F("CenteraletaVphi",       ";Most central jet |#eta|;#phi",30,-4.7,4.7,25,-TMath::Pi(),TMath::Pi()));
  ht->addHist("forwardetaVpuMva",       new TH2F("forwardetaVpuMva",       ";Most forward jet |#eta|;PUMVA",30,-4.7,4.7,25,-1,1));
  ht->addHist("forwardetaVpuMva",       new TH2F("centeraletaVpuMva",       ";Most central jet |#eta|;PUMVA",30,-4.7,4.7,25,-1,1));
  ht->addHist("etaphi",           new TH2F("etaphi",       ";jet |#eta|;#phi",30,-4.7,4.7,25,-TMath::Pi(),TMath::Pi()));
...
void VBFVectorBoson::fill(MiniEvent_t ev, TLorentzVector boson, std::vector<Jet> jets, std::vector<double> cplotwgts, TString c, std::map<TString, int> mults, std::vector<Particle> fakeACR, std::vector<Particle> tightACR){
  ...
  for(size_t ij=0; ij<min(size_t(2),jets.size());ij++) {
    ...
    j_Pumva = jets[ij].getPUMVA();
    j_phi= jets[ij].Phi();
    LeadingJet= jets[0].Pt();
    SubLeadingJet=jets[1].Pt();
    
    ht->fill("leadpt",    LeadingJet 	  ,  cplotwgts,c);
    ht->fill("subleadpt",    SubLeadingJet	  ,  cplotwgts,c);
  ...
  ht->fill2D("CenteraletaVphi",    centraleta,j_phi ,   cplotwgts,c);
  ht->fill2D("forwardetaVpuMva",    forwardeta,j_Pumva ,   cplotwgts,c);
  ht->fill2D("CenteraletaVpuMva",    centraleta,j_Pumva ,   cplotwgts,c);
